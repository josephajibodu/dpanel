...
export default function SiteDeployScriptShow({
    server: serverProp,
    site: siteProp,
}: Props) {
    ...
    function handleSubmit(e: React.FormEvent) {
        e.preventDefault();
        form.put(
            teamPath(`/servers/${serverId}/sites/${site.id}/deploy-script`),
        );
    }
    ...
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        },
        {
            title: 'Deploy Script',
            href: teamPath(
                `/servers/${serverId}/sites/${site.id}/deploy-script`,
            ),
        },
    ];

    const editorTheme =
        resolvedAppearance === 'dark' ? 'shell-dark' : 'shell-light';
            ...
            <div className="flex h-full flex-1 flex-col gap-6 p-4">
                <div>
                    ...
                    <p className="mt-1 text-sm text-muted-foreground">
                        Customize the deployment script that runs when you
                        deploy your site. Available variables: $SITE_ROOT,
                        $BRANCH, $PHP, $COMPOSER, $PHP_FPM
                    </p>
                </div>
