...
interface SiteProvisioningStepTimelineProps {
    currentStep: { value: number; label: string; description: string } | null;
    steps: ProvisioningStep[];
    className?: string;
}

export function SiteProvisioningStepTimeline({
    currentStep,
    steps,
    className,
}: SiteProvisioningStepTimelineProps) {
    const currentStepValue = currentStep?.value ?? 0;
    const currentStepLabel = currentStep?.label ?? 'Pending';
    const currentStepDescription =
        currentStep?.description ?? 'Waiting to start installation.';
